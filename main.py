...
async def read_text(request: ImageUploadRequest):
    ...
    Gelen base64 görüntüyü çözer, PIL Image'e çevirir ve
    pytesseract ile (varsa) Türkçe + İngilizce metin okur.
    """
    logger.info("OCR request received")

    # Eğer pytesseract kurulmamışsa eski mock davranışına dön
    if pytesseract is None:
        logger.warning("pytesseract not available, returning mock OCR text")
        return OcrResponse(text="VisionGuide ile hayatı keşfet")
    ...
    try:
        ...
        # Gerekirse gri tona çevir (OCR için bazen daha iyi sonuç verir)
        if image.mode != "L":
            image = image.convert("L")

        # Türkçe + İngilizce OCR dene. Sunucuda uygun dil paketleri olmalı (tur, eng).
        try:
            text = pytesseract.image_to_string(image, lang="tur+eng")
        except pytesseract.TesseractError as e:
            logger.error(f"Tesseract error: {e}")
            # Dil paketi yoksa varsayılan dil ile tekrar dene
            text = pytesseract.image_to_string(image)

        # Temizlik: baştaki/sondaki boşlukları at
        text = text.strip()
        if not text:
